...
@interface DiekScrollAndPageView(){
    UIView *_firstView;
    UIView *_middleView;
    UIView *_lastView;
...
@implementation DiekScrollAndPageView
...
//        if (!_autoScrollTimer) {
                _autoScrollTimer = [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(autoShowNextImage) userInfo:nil repeats:YES];
//        }else{
//            [_autoScrollTimer setFireDate:[NSDate date]]; //继续。
//        }
    }
    else   //关闭自动翻页
    {
        if (_autoScrollTimer.isValid) {
            [_autoScrollTimer invalidate];
            [_autoScrollTimer setFireDate:[NSDate distantFuture]];//暂停
            _autoScrollTimer = nil;
        }
    }
}
